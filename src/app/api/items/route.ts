...
const schema = z.object({
  ...
  existing: z.array(z.string().trim().min(1).max(100)).max(30).default([]),
...
export async function POST(req: Request) {
  ...
  const requestedCount = Math.min(count ?? 30, 30 - existing.length);
  if (requestedCount < 1) return NextResponse.json({ items: [] });
